function procesarLista() {
    ...
    resultadoElement.innerHTML = `mutation admPosMovil {<br>`;
    for (let i=0; i < listaCadenas.length; i++) {
        ...
        resultadoElement.innerHTML += `&emsp;<span><b>m${i}</b>: associateUserToClient(user: "<b>${usuarios}</b>", clientId: "pos_movil_control_panel"),</span><br>`;

        resultadoElement.innerHTML += `&emsp;<span>p${i}: associatePermission(user: "<b>${usuarios}</b>", clientId: "pos_movil_control_panel", scope: <b>"${scope}"</b>),</span><br>`;
    }
