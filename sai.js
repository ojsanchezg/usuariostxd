function procesarLista() {
    ...
    resultadoElement.innerHTML = `mutation sai {<br>`;
    for (let i=0; i < listaCadenas.length; i++) {
        ...
        resultadoElement.innerHTML += `&emsp;<span><b>m${i}</b>: associateUserToClient(user: "<b>${usuarios}</b>", clientId: "sai-front"),</span><br>`;

        resultadoElement.innerHTML += `&emsp;<span>p${i}: associatePermission(user: "<b>${usuarios}</b>", clientId: "sai-front", scope: <b>"${scope}"</b>),</span><br>`;
    }
