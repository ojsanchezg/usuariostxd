function procesarLista() {
    ...
    resultadoElement.innerHTML = `mutation insecurePickersOdeon {`;
    for (let i=0; i < listaCadenasNombre.length; i++) {
        ...
        resultadoElement.innerHTML += `<p><b>m${i}</b>: associateByRut(user: <b>"${usuariosRut}"</b>, firstName: <b>"${usuarios}"</b>, lastName: <b>"${usuariosApellidos}"</b>, clientId: "app_tienda_mobile")</p>`;
        resultadoElement.innerHTML += `<p>p${i}: associatePermission(user: <b>"${usuariosRut}"</b>, clientId: "app_tienda_mobile", scope: "odeon-picker", metadata: stores::<b>${cadenaTiendaUsuarios}"</b>)</p>`;
    }
